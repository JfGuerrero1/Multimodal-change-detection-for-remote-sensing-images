...
class GradualExpansionUNet(nn.Module):
    """
    Gradual Expansion U-Net for Ordinary MSI to HSI reconstruction.
    Input:  [B, in_msi, H, W]   (e.g., [B, 12, H, W])
    Output: [B, in_hsi, H, W]   (e.g., [B, 230, H, W])
    """
    def __init__(self, in_msi, in_hsi, base_channel=64, interpolation_mode='Bilinear', 
        ...
        if final_activation and final_activation.lower() not in ['relu', 'softplus']:
        ...
        self.up2b = DoubleConv(4 * base_channel + base_channel * 2, 4 * base_channel, activation, with_batch_norm)
    ...
    def forward(self, x):
        ...
        out = self.outc(x)            # [B, in_hsi, H, W]       (Linear Output)
