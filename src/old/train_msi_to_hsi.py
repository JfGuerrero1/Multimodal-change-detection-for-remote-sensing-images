...
def main():
    ...
    parser.set_defaults(is_residual=False) # Par défaut à False si non spécifié
...
# Vérification rapide
    ...
    run_name = f"{args.model}_{mode_str}_{aug_str}_SAM-{args.lambda_sam}_lr-{args.lr}_final"

    wandb.init(
    entity="JfGuerrero",                        # Ton username exact
    project="Multimodal-change-detection",      # Le nom exact du projet
    name=run_name,                 # Le nom de ce run dans ton tableau de bord
    config=vars(args)                           # Sauvegarde automatiquement (lr, batch_size, epochs...)
    )


    for epoch in range(args.epochs):

        train_loss, train_mse, train_mae, train_sam = train_one_epoch(
        ...
        # Envoie les scores de l'époque actuelle directement à WandB
        wandb.log({
        ...
        if val_loss < best_val_loss:
            ...
            mode_str = "sim" if args.simulated else "real"
            aug_str = "aug" if args.augment else "noaug"
            model_name = f"best_{args.model}_SAM-{args.lambda_sam}_lr-{args.lr}_{mode_str}_{aug_str}_final.pth"
