...
from models import UNet, GradualExpansionUNet_residual
...
@torch.no_grad()
def test_model_metric(model,loader,device,save_dir):
    ...
    for x, x_interp, y in pbar: #(MSI,HSI_interp, HSI_gt)
        x=x.to(device)
        y=y.to(device)
        x_interp=x_interp.to(device)
        pred=model(x,x_interp)
        pred=pred.to(device) #HSI_pred
    ...
    print("="*50)
    print(f"SAM Global Moyen  : {sam_moyen:.2f} (± {sam_std:.2f})rad")
    print("="*50)
    ...
    #
    sam_stats = {
        "sam_moyen_degres": float(np.mean(array_sam)),
        "sam_std_degres": float(np.std(array_sam))
                 }
...
def main():
    ...
    print("Modele residuel")
    poids_res= "/home/ids/jfguerrero/Multimodal-change-detection-for-remote-sensing-images/models/gradualexpansionunet_res_real_aug_SAM-0.5_lr-0.0005_final.pth" 
    model_res = GradualExpansionUNet_residual(in_msi=12, in_hsi=230, interpolation_mode="Bilinear", activation="silu")
    state_dict_res = torch.load(poids_res, map_location=device)
    ...
    save_dir=RESULT_DIR/run_name
    test_model_metric(model_res, test_loader, device, save_dir)
